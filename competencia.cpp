...
void TORNEO::GANADOR()
{
					...
					PPAR1 = INICIO;
					 TOTAL = 0;
						...
						{	
							...
							while(JUG)
							{	
								if( strcmp ( PAL -> NOM , JUG -> NOM ) == 0 )
									
									TOTAL += (PPAR1 -> HANDICAP) * PUNTT ;
		...
		printf(" el ganador es %s con %f puntos \n ", GAN , MAYOR);
		
		INFO(GAN);
}

void TORNEO::INFO(char S [])		
{
	DISCIPLINA * PPAR;
	COMPETIDOR * PAL;
		int POS;
		...
		while (PPAR)
		{
			...
			while (PAL)
			{
				if(strcmp(PAL->NOM , S)==0)
				{
					printf(" %s salio %d \n ", PPAR -> NOM , POS);
					TOTAL += ( PPAR -> HANDICAP ) * 1000;
				}
				POS ++;
				PAL = PAL->SIG;
			}
	...
	printf( " se gasto un total de : %f", TOTAL);
}	
